        ...
        <div className="px-6 md:px-10 xl:px-28 py-20">
            <div className="mb-12 xl:mb-0 max-w-[45rem]">
                <h2 className="text-[2rem] leading-[2.5rem] sm:text-[2.2rem] sm:leading-[2.5rem] lg:text-[2.5rem] lg:leading-[3rem]">
                    <span className="text-[#60a6e7] capitalize">From Spark to Spotlight:</span> we take you every step of the way to success.
                </h2>
            </div>
            <div className="flex lg:items-end flex-col md:flex-row">
                <div className="basis-1/2 flex mb-8 md:mb-0">
                    <div className="md:pr-10">
                        <div className="border-2 border-primary mb-12 rounded-full grid gap-1 grid-cols-4 p-3.5">
                            {steps.map((step, index) => (
                                <div
                                    key={step.name}
                                    onClick={() => setActiveStep(index)}
                                    className={`px-2 py-3 text-center font-medium cursor-pointer transition-all duration-300 ${
                                        activeStep === index
                        ...
                        <a href="#" className="relative group">
                            <button className="bg-white px-5 py-3 rounded-full group-hover:bg-[#064386] text-bg font-semibold group-hover:text-white transition-all duration-300 ease">
                                Visit Blog <FaCaretRight className="inline-block ml-0.5 text-lg -translate-y-0.5 group-hover:hidden" />
                            </button>
